...
router.get('/microsoft/callback', async (req, res) => {
    try {
        ...
        if (error) {
            ...
            return res.redirect(
                `${process.env.FRONTEND_URL || 'http://localhost:8090'}/profile?error=microsoft_auth_failed`
            );
        }
        ...
        if (!code) {
            ...
            return res.redirect(
                `${process.env.FRONTEND_URL || 'http://localhost:8090'}/profile?error=no_auth_code`
            );
        }
        ...
        if (!userId) {
            ...
            return res.redirect(
                `${process.env.FRONTEND_URL || 'http://localhost:8090'}/profile?error=no_user_session`
            );
        }
...
router.get('/microsoft/status', async (req, res) => {
    try {
        ...
        logInfo(
            `[DEBUG] Microsoft Todo status check for user ${req.session.userId}:`,
            {
        ...
        if (isConnected && user.microsoft_todo_expires_at) {
            ...
            if (now >= expiresAt) {
                ...
                try {
                    ...
                    logError(
                        'Failed to refresh Microsoft Todo token',
                        refreshError
                    );
