...
router.post('/import', async (req, res) => {
    try {
        ...
        const result = await microsoftTodoService.importTasksFromMicrosoft(req.currentUser.id, access_token, forceUpdate);
        
        logInfo(`Microsoft ToDo import completed for user ${req.currentUser.id}: ${result.imported} tasks imported (forceUpdate: ${forceUpdate})`);
        res.json({ 
            success: true, 
...
router.post('/sync', async (req, res) => {
    try {
        ...
        const importResult = await microsoftTodoService.importTasksFromMicrosoft(req.currentUser.id, access_token, forceUpdate);
        const exportResult = await microsoftTodoService.exportTasksToMicrosoft(req.currentUser.id, access_token);
        
        logInfo(`Microsoft ToDo sync completed for user ${req.currentUser.id}: ${importResult.imported} imported, ${exportResult.exported} exported (forceUpdate: ${forceUpdate})`);
        res.json({ 
            success: true, 
