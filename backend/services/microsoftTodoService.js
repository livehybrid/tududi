...
class MicrosoftTodoService {
    constructor() {
        ...
        console.log(
            '[MICROSOFT SERVICE] Client Secret:',
            this.clientSecret ? '***SET***' : 'NOT SET'
        );
    ...
    async getAccessToken(authCode) {
        try {
            if (!this.clientSecret) {
                throw new Error(
                    'Microsoft client secret is not configured. Please set MICROSOFT_CLIENT_SECRET environment variable.'
                );
            }
    ...
    async getTodoLists(accessToken) {
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    `Failed to fetch ToDo lists: ${response.statusText}`
                );
            }
    ...
    async getTasksFromList(accessToken, listId) {
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    `Failed to fetch tasks from list ${listId}: ${response.statusText}`
                );
            }
            ...
            logError(
                `Failed to fetch tasks from Microsoft ToDo list ${listId}`,
                error
            );
            throw error;
        }
    ...
    mapMicrosoftTaskToTududi(microsoftTask, projectId, userId) {
        ...
        const mappedTask = {
            ...
            due_date: microsoftTask.dueDateTime?.dateTime
                ? new Date(microsoftTask.dueDateTime.dateTime)
                : null,
            priority: priority,
            status: status,
            ...
            completed_at: microsoftTask.completedDateTime?.dateTime
                ? new Date(microsoftTask.completedDateTime.dateTime)
                : null,
        };
    ...
    mapTududiTaskToMicrosoft(tududiTask) {
        ...
        logInfo(`[DEBUG] Tududi Task Fields for "${tududiTask.name}":`, {
            ...
            created_at:
                tududiTask.created_at &&
                tududiTask.created_at !== 'Invalid Date'
                    ? tududiTask.created_at.toISOString()
                    : 'NULL',
            updated_at:
                tududiTask.updated_at &&
                tududiTask.updated_at !== 'Invalid Date'
                    ? tududiTask.updated_at.toISOString()
                    : 'NULL',
    ...
    async importTasksFromMicrosoft(userId, accessToken, forceUpdate = false) {
        try {
            ...
            for (const list of lists) {
                ...
                logInfo(
                    `Found ${tasks.length} tasks in list: ${list.displayName}`
                );
                ...
                for (const microsoftTask of tasks) {
                    ...
                    const existingTask = await Task.findOne({
                        ...
                        attributes: [
                            ...
                            'created_at',
                            'updated_at', // Explicitly include timestamp fields
                        ],
                    });

                    if (existingTask) {
                        // Map Microsoft task to Tududi format first
                        const updatedTaskData = this.mapMicrosoftTaskToTududi(
                            microsoftTask,
                            projectId,
                            userId
                        );
                        ...
                        const hasContentChanges =
                            ...
                            existingTask.due_date?.getTime() !==
                                updatedTaskData.due_date?.getTime() ||
                            existingTask.priority !==
                                updatedTaskData.priority ||
                            existingTask.status !== updatedTaskData.status ||
                            existingTask.completed_at?.getTime() !==
                                updatedTaskData.completed_at?.getTime();

                        // Also check timestamp comparison for debugging
                        const msUpdated = new Date(
                            microsoftTask.lastModifiedDateTime
                        );
                        // Use updated_at if available, otherwise fall back to external_last_modified
                        const localUpdated =
                            existingTask.updated_at &&
                            existingTask.updated_at !== 'Invalid Date'
                                ? new Date(existingTask.updated_at)
                                : existingTask.external_last_modified
                                  ? new Date(
                                        existingTask.external_last_modified
                                    )
                                  : null;
                        const isMsNewer = localUpdated
                            ? msUpdated > localUpdated
                            : true; // If no local timestamp, assume MS is newer

                        logInfo(
                            `[DEBUG] Task "${microsoftTask.title}" update check:`,
                            {
                                hasContentChanges,
                                isMsNewer,
                                msUpdated: msUpdated.toISOString(),
                                localUpdated: localUpdated
                                    ? localUpdated.toISOString()
                                    : 'NULL',
                                localCreatedAt:
                                    existingTask.created_at &&
                                    existingTask.created_at !== 'Invalid Date'
                                        ? existingTask.created_at.toISOString()
                                        : 'NULL',
                                localUpdatedAt:
                                    existingTask.updated_at &&
                                    existingTask.updated_at !== 'Invalid Date'
                                        ...
                                        : 'NULL',
                                localExternalLastModified:
                                    existingTask.external_last_modified
                                        ? existingTask.external_last_modified.toISOString()
                                        : 'NULL',
                                timestampSource:
                                    existingTask.updated_at &&
                                    existingTask.updated_at !== 'Invalid Date'
                        ...
                        if (hasContentChanges || isMsNewer || forceUpdate) {
                            ...
                            logInfo(
                                `[DEBUG] Updating task "${microsoftTask.title}" - Content changes: ${hasContentChanges}, MS newer: ${isMsNewer}, Force update: ${forceUpdate}`
                            );
                            ...
                            if (existingTask.name !== updatedTaskData.name) {
                                changes.push({
                                    field: 'name',
                                    old: existingTask.name,
                                    new: updatedTaskData.name,
                                });
                            }
                            if (existingTask.note !== updatedTaskData.note) {
                                changes.push({
                                    field: 'note',
                                    old: existingTask.note,
                                    new: updatedTaskData.note,
                                });
                            }
                            if (
                                existingTask.due_date?.getTime() !==
                                ...
                                changes.push({
                                    field: 'due_date',
                                    old: existingTask.due_date,
                                    new: updatedTaskData.due_date,
                                });
                            }
                            if (
                                existingTask.priority !==
                            ...
                            if (
                                ...
                                changes.push({
                                    field: 'completed_at',
                                    old: existingTask.completed_at,
                                    new: updatedTaskData.completed_at,
                                });
                            }
                            ...
                            logInfo(
                                `Updated existing task from Microsoft: ${microsoftTask.title}`
                            );
                        } else {
                            ...
                            if (localUpdated && localUpdated > msUpdated) {
                                ...
                                logInfo(
                                    `Local task is newer, will update Microsoft: ${existingTask.name}`
                                );
                                tasksToUpdate.push(microsoftTask.id);
                            } else {
                                logInfo(
                                    `Skipping task ${microsoftTask.title} - no updates needed (timestamps equal and no content changes)`
                                );
                            }
                        }
                    } else {
                        // Create new task
                        const tududiTask = this.mapMicrosoftTaskToTududi(
                            microsoftTask,
                            projectId,
                            userId
                        );
                ...
                if (tasksToCreate.length > 0) {
                    ...
                    logInfo(
                        `Created ${tasksToCreate.length} new tasks from list: ${list.displayName}`
                    );
                ...
                if (tasksToUpdate.length > 0) {
                    ...
                    logInfo(
                        `Updated ${tasksToUpdate.length} existing tasks from list: ${list.displayName}`
                    );
                }
            }

            logInfo(
                `Microsoft ToDo import completed: ${totalImported} tasks imported from ${lists.length} lists`
            );
    ...
    async exportTasksToMicrosoft(userId, accessToken) {
        try {
            ...
            for (const task of tasks) {
                try {
                    ...
                    if (task.external_id && task.external_last_modified) {
                        ...
                        const externalUpdated = new Date(
                            task.external_last_modified
                        );

                        // Skip if local task is not newer than external
                        if (localUpdated <= externalUpdated) {
                            logInfo(
                                `Skipping task ${task.name} - no local updates since last sync`
                            );
                            continue;
                        }
                    }
                    ...
                    if (task.project && task.project.name) {
                        ...
                        targetListId = listMap.get(
                            targetListName.toLowerCase()
                        );
                        ...
                        if (!targetListId) {
                            ...
                            if (createListResponse.ok) {
                                ...
                                logInfo(
                                    `Created new Microsoft ToDo list: ${targetListName}`
                                );
                            }
                        }
                    }
                    ...
                    if (response.ok) {
                        ...
                        exportedCount++;
                        logInfo(
                            `Exported task: ${task.name} to list: ${targetListName}`
                        );
                    } else {
                        logError(
                            `Failed to export task ${task.name}: ${response.statusText}`
                        );
                    }
                } catch (error) {
            ...
            logInfo(
                `Microsoft ToDo export completed: ${exportedCount} tasks exported`
            );
