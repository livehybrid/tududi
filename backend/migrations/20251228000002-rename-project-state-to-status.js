...
module.exports = {
    async up(queryInterface, Sequelize) {
        ...
        if (tableInfo.status) {
            ...
            console.log(
                '⚠️  Column is already named "status", skipping rename'
            );
            return;
        }
        ...
        if (tableInfo.state) {
            ...
            console.log(
                '⚠️  Neither "state" nor "status" column found, skipping rename'
            );
        }
    },
    ...
    async down(queryInterface, Sequelize) {
        ...
        if (tableInfo.state) {
            ...
            console.log(
                '⚠️  Column is already named "state", skipping rollback'
            );
            return;
        }
        ...
        if (tableInfo.status) {
            ...
            console.log(
                '⚠️  Neither "state" nor "status" column found, skipping rollback'
            );
        }
    },
};
